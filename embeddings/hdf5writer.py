...
class embedding_writer:
    def __init__(self, filename: str, model_name: str, num_rows: int, num_rows_avg: int, num_cols: int, dtype='f4', avg = True):
        ...
        self.avg = avg
        if self.avg:
            self.dataset_avg = self.h5writer.add_dataset(f'{model_name}_averages', num_rows_avg, num_cols - 2, dtype)
    ...
    def writerows(self, filename, centers_i, centers_j, embeddings):
        ...
        if self.avg:
            embeddings_avg = np.mean(embeddings, axis=0, keepdims=True)
            self.dataset_avg.write_rows(np.concatenate(([[file_idx]], embeddings_avg), axis=1))
