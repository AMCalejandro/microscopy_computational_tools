...
for plate in plates:
    ...
    j.command('git clone -b averages --single-branch https://github.com/AMCalejandro/microscopy_computational_tools.git')
    ...
    j.command(f'pixi run python embeddings/run_model.py {args.model} {model_weights} /images/{quote(image_folder)} {quote(args.channel_names)} {quote(args.channel_substrings)} {quote(centers_file)} {num_workers} embedding.h5 crops.png {args.averages}')
